...
WebUI.initWidgets = function() {
    ...
    WebUI.btn_cancle=new WebUI.PushButton("Cancel", {width:100, height:50});
...
WebUI.layoutWidgets = function() {
    ...
    WebUI.btn_cancle.moveTo({left:160, top:350});
}
...
WebUI.Switch.prototype.initVisualItems = function() {
    ...
    let start_1_x = this.position.left + radius;
    let start_1_y=this.position.top;    
    let end_1_x= this.position.left + 3*radius;
    let end_1_y=start_1_y;

    let end_2_x=start_1_x;
    let end_2_y= this.position.top+2*radius;

    let centerX = this.position.left + 3*(this.desired_size.width/4);
    let centerY = this.position.top + this.desired_size.width/4;
   
    let start_angle= -Math.PI/2 ;
    let end_angle= Math.PI/2 - 0.01;
    ...
    for(; start_angle < end_angle ; start_angle += 0.01){
        path_arc1.push(' L ');
        path_arc1.push(centerX+radius*Math.cos(start_angle));
        path_arc1.join(' ');
        path_arc1.push(centerY+radius*Math.sin(start_angle));
    }
    path.push(path_arc1);
    ...
    let centerX_2 = this.position.left + radius;
    let centerY_2 = this.position.top + radius;
   
    let start_angle_2= Math.PI/2 ;
    let end_angle_2= (3/2)*Math.PI -0.01;

    for(; start_angle_2 < end_angle_2 ; start_angle_2 += 0.01){
        path_arc2.push(' L ');
        path_arc2.push(centerX_2 + radius*Math.cos(start_angle_2));
        path_arc2.join(' ');
        path_arc2.push(centerY_2 + radius*Math.sin(start_angle_2));
    }
    path.push(path_arc2);
    ...
    let boundary = new fabric.Path(path);
    boundary.set({
        left: this.position.left,
        top: this.position.top,
        width: this.desired_size.width,
...
WebUI.Switch.prototype.handleMouseDown = function() {
    ...
    let circle = this.visual_items[1];
    let boundary = this.visual_items[0];

    const centerx_off = boundary.left + 3*this.desired_size.width/4;
    const centerx_on = boundary.left + this.desired_size.width/4;
    
    //is.on == false 일 때 
     if(this.is_on == false){ 

        boundary.set('fill','rgb(48,209,88)'); //초록
        boundary.set('stroke','rgb(48,209,88)');
        circle.set('stroke','rgb(48,209,88)');

        circle.animate('left',centerx_off,
        {
    ...
    //is.on == true 일 때
        ...
        boundary.set('fill','rgb(142,142,147)'); //회색
        boundary.set('stroke','rgb(142,142,147)');
        circle.set('stroke','rgb(142,142,147)');

        circle.animate('left',centerx_on,
        {
            onChange: WebUI.canvas.renderAll.bind(WebUI.canvas),
            duration: 100,
